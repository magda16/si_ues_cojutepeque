...
$("#proveedor").blur(function(){
  ...
  var validarcampo = $("#vproveedor").val();
...
$("#telefono_p").blur(function(){
  ...
  var validarcampo = $("#vtelefono").val();
  if(nombre !== validarcampo){
  var tabla = "proveedor";
...
$("#correo_p").blur(function(){
  ...
  if(nombre.length>2){
      ...
      .done(function(resultado_ajax){
        if(resultado_ajax!==""){
          $("#correo_p").val("");
          $('#resulcorerror').text("Correo Ya Existe");
          $('#resulcor').removeClass('has-success').addClass('has-error');
        }
      })
      .fail(function(){
        alert('Hubo un error al cargar la Pagina')
